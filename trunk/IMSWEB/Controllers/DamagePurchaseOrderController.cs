...
{
    [Authorize]
    [RoutePrefix("damagepurchaseorder")]
    public class DamagePurchaseOrderController : CoreController
    {
        ...
        public DamagePurchaseOrderController(IErrorService errorService,
            ...
            IProductService productService, ISisterConcernService SisService
            )
            : base(errorService)
        {
        ...
        public async Task<ActionResult> Index()
        {
            ...
            var customPO = _purchaseOrderService.GetAllDamagePurchaseOrderAsync(DateRange.Item1, DateRange.Item2);
        ...
        public async Task<ActionResult> Index(FormCollection formCollection)
        {
            ...
            var customPO = _purchaseOrderService.GetAllDamagePurchaseOrderAsync(ViewBag.FromDate, ViewBag.ToDate);
            var vmPO = _mapper.Map<IEnumerable<Tuple<int, string, DateTime, string, string, string, EnumPurchaseType>>,
                IEnumerable<GetPurchaseOrderViewModel>>(await customPO);
        ...
        public ActionResult Delete(int orderId)
        {
            ...
            if (_purchaseOrderService.DeleteDamagePurchaseOrderUsingSP(orderId, User.Identity.GetUserId<int>()))
        ...
        private void SaveOrder(PurchaseOrderViewModel newPurchaseOrder, PurchaseOrderViewModel purchaseOrder, FormCollection formCollection, bool IsDeliveryOrder)
        {
            ...
            if (IsDeliveryOrder)
                ...
                purchaseOrder.PurchaseOrder.Status = Convert.ToString((int)EnumPurchaseType.DamagePurchase);
            ...
            else
            {
                ...
                if (ControllerContext.RouteData.Values["action"].ToString().ToLower().Equals("edit"))
                {
                    Result = _purchaseOrderService.UpdateDamagePurchaseOrderUsingSP(int.Parse(purchaseOrder.PurchaseOrder.PurchaseOrderId), dtPurchaseOrder, dtPurchaseOrderDetail, dtPOProductDetail, dtStock, dtStockDetail);
                    if (Result)
                    {
                        TempData["POrderID"] = int.Parse(purchaseOrder.PurchaseOrder.PurchaseOrderId);
                        TempData["IsInvoiceReadyById"] = true;
                ...
                {
                    Result = _purchaseOrderService.AddDamagePurchaseOrderUsingSP(dtPurchaseOrder, dtPurchaseOrderDetail, dtPOProductDetail, dtStock, dtStockDetail);
                    if (Result)
                    {
                        POrder pOrder = new POrder();
                        pOrder = _mapper.Map<CreatePurchaseOrderViewModel, POrder>(purchaseOrder.PurchaseOrder);
